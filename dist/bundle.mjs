...
class k extends HTMLElement {
  ...
  connectedCallback() {
    this.btn = this.shadowRoot.getElementById("btn"), this.ico = this.shadowRoot.getElementById("icon"), this.load = document.createElement("xy-loading"), this.load.style.color = "inherit", this.btn.addEventListener("mousedown", (t) => {
      if (!this.disabled) {
        const {
          left: o,
          top: e
        } = this.getBoundingClientRect();
        this.style.setProperty("--x", `${t.clientX - o}px`), this.style.setProperty("--y", `${t.clientY - e}px`);
      }
  ...
  attributeChangedCallback(t, o, e) {
    t === "disabled" && this.btn && (e !== null ? (this.btn.setAttribute("disabled", "disabled"), this.href && this.btn.removeAttribute("href")) : (this.btn.removeAttribute("disabled"), this.href && (this.btn.href = this.href))), t === "loading" && this.btn && (e !== null ? (this.shadowRoot.prepend(this.load), this.btn.setAttribute("disabled", "disabled")) : (this.shadowRoot.removeChild(this.load), this.btn.removeAttribute("disabled"))), t === "icon" && this.ico && (this.ico.name = e), t === "href" && this.btn && (this.disabled || (this.btn.href = e)), t === "htmltype" && this.btn && (this.btn.type = e);
  }
}
...
class p extends HTMLElement {
  ...
  attributeChangedCallback(t, o, e) {
    t === "loading" && this.shadowRoot && (e !== null ? this.btnSubmit.loading = !0 : this.btnSubmit.loading = !1), t === "title" && this.titles && e !== null && (this.titles.innerHTML = e), t === "oktext" && this.btnSubmit && e !== null && (this.btnSubmit.innerHTML = e), t === "canceltext" && this.btnCancel && e !== null && (this.btnCancel.innerHTML = e);
  }
}
customElements.get("xy-popcon") || customElements.define("xy-popcon", p);
class A extends HTMLElement {
  ...
  show(t) {
    ...
    else if (this.popcon || (this.popcon = new p(this.type), this.popcon.type = this.type, this.appendChild(this.popcon), this.popcon.title = this.title || "popover", this.popcon.innerHTML = this.content || "", this.type === "confirm" && (this.popcon.oktext = this.oktext || "confirm", this.popcon.canceltext = this.canceltext || "cancel", this.popcon.onsubmit = () => this.dispatchEvent(new CustomEvent("submit")), this.popcon.oncancel = () => this.dispatchEvent(new CustomEvent("cancel")))), this.trigger === "contextmenu") {
      const {
        x: o,
        y: e
      } = this.getBoundingClientRect();
      this.popcon.style.setProperty("--x", `${t.clientX - o}px`), this.popcon.style.setProperty("--y", `${t.clientY - e}px`), this.popcon.open = !0;
    } else
  ...
  connectedCallback() {
    this.popcon = this.querySelector("xy-popcon"), this.trigger && this.trigger !== "click" || this.addEventListener("click", this.show), this.trigger === "contextmenu" && this.addEventListener("contextmenu", (t) => {
      ...
      (this.popcon && !o.includes(this.popcon) && !this.popcon.loading && !o.includes(this.children[0]) || this.trigger === "contextmenu" && !o.includes(this.popcon) && t.which === "1") && (this.popcon.open = !1);
    });
  }
  attributeChangedCallback(t, o, e) {
    t === "loading" && this.popcon && (e !== null ? this.popcon.loading = !0 : this.popcon.loading = !1), t === "title" && this.popcon && e !== null && (this.popcon.title = e), t === "oktext" && this.popcon && e !== null && (this.popcon.oktext = e), t === "canceltext" && this.popcon && e !== null && (this.popcon.canceltext = e);
  }
}
...
function u(i, t) {
  const o = sessionStorage.getItem(`${c}-${t}`);
  return o ? JSON.parse(o) : i;
}
function g(i, t) {
...
const f = "ce-inline-toolbar__dropdown", m = "ce-conversion-toolbar--showed", S = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  CONVERTER_BTN: f,
  CONVERTER_PANEL: m,
  getCustomColorCache: b,
  getDefaultColorCache: u,
  handleCSSVariables: l,
  setCustomColorCache: g,
  setDefaultColorCache: h,
  throttle: d
}, Symbol.toStringTag, { value: "Module" })), L = ["#ff1300", "#EC7878", "#9C27B0", "#673AB7", "#3F51B5", "#0070FF", "#03A9F4", "#00BCD4", "#4CAF50", "#8BC34A", "#CDDC39", "#FFE500", "#FFBF00", "#FF9800", "#795548", "#9E9E9E", "#5A5A5A", "#FFF"];
class y extends HTMLElement {
  ...
  constructor(t = {}) {
    super();
    const o = this.attachShadow({ mode: "open" });
    this.colorCollections = t.colorCollections || L, this.onColorPicked = t.onColorPicked, this.defaulColor = l(t.defaultColor || this.colorCollections[0]), this.pluginType = t.type, this.hasCustomPicker = t.hasCustomPicker, this.customColor = b(this.pluginType), o.innerHTML = `
        <style>
        :host{
            display:inline-block;
            width:15px;
  ...
  connectedCallback() {
    this.$popover = this.shadowRoot.getElementById("popover"), this.popcon = this.shadowRoot.getElementById("popcon"), this.colorBtn = this.shadowRoot.getElementById("color-btn"), this.colors = this.shadowRoot.getElementById("colors"), this.colors.addEventListener("click", (t) => {
      ...
      o && o.id !== "custom-picker" && (this.nativeclick = !0, this.value = l(o.dataset.color), this.onColorPicked(this.value));
  ...
  closeConverter() {
    if (document.getElementsByClassName(m)[0]) {
      const o = document.getElementsByClassName(f)[0];
      o == null || o.click();
    }
  }
  ...
  setupCustomPicker() {
    ...
    const { customPicker: o } = this;
    o.style.backgroundColor = this.customColor, this.customPicker.addEventListener("click", () => {
      if (t) {
        t = !1;
        return;
      }
      this.pickerInput && document.body.removeChild(this.pickerInput), this.pickerInput = document.createElement("input");
      const { pickerInput: e } = this, s = this.popcon.getBoundingClientRect();
      e.setAttribute("type", "color"), e.value = this.customColor, e.style.position = "fixed", e.style.left = `${s.x + 3}px`, e.style.top = `${s.y + 10}px`, e.style.pointerEvents = "none", e.style.zIndex = "999", e.style.opacity = "0", e.addEventListener("input", d((r) => {
        this.nativeclick = !0, this.value = l(r.target.value), this.onColorPicked(this.value), g(this.value, this.pluginType), o.style.backgroundColor = this.value, t = !0, o.click();
      }, 30)), document.body.appendChild(e), setTimeout(() => {
        e.focus(), e.click();
      }, 0);
    });
  }
  ...
  set value(t) {
    t && (this.$value = t, this.colorBtn.style.setProperty(
      "--themeColor",
      this.nativeclick ? h(t, this.pluginType) : u(t, this.pluginType)
...
class q {
   ...
   */
  createLeftButton() {
    return this.icon || (this.icon = document.createElement("div"), this.icon.id = "color-left-btn", this.icon.appendChild(this.createButtonIcon()), this.icon.addEventListener("click", () => {
      this.clickedOnLeft = !0;
    })), this.icon;
  }
  /**
   * Create button icon
  ...
  createButtonIcon() {
    ...
    t.id = "color-btn-text";
    const o = this.pluginType === "marker" ? H : F;
    return t.innerHTML = this.config.icon || o, t;
  }
  /**
  ...
  surround(t) {
    if (!t)
      return;
    const o = this.api.selection.findParentTag("SPAN");
    o && this.unwrap(o);
    const e = this.api.selection.findParentTag(this.parentTag);
    e ? this.unwrap(e) : this.wrap(t), this.clickedOnLeft = !1;
  }
  /**
   *
   * @param {Range} range;
   */
  unwrapTagInRange(t, o) {
    o = o.toUpperCase();
    const { startContainer: e } = t, { endContainer: s } = t;
    e.nodeType === Node.TEXT_NODE && e.parentNode.tagName === o && t.setStartBefore(e.parentNode), s.nodeType === Node.TEXT_NODE && s.parentNode.tagName === o && t.setEndAfter(s.parentNode);
    const r = t.extractContents(), x = r.querySelectorAll(o), v = (n) => {
      for (; n.firstChild; )
        n.parentNode.insertBefore(n.firstChild, n);
      n.parentNode.removeChild(n);
    };
    x.forEach((n) => v(n)), t.deleteContents(), t.insertNode(r);
  }
  /**
  ...
  wrap(t) {
    ...
    const o = t.extractContents(), e = document.createElement(this.parentTag);
    e.appendChild(o), t.insertNode(e), this.pluginType === "marker" ? this.wrapMarker(e) : this.wrapTextColor(e), this.api.selection.expandToTag(e);
  }
  /**
  ...
  unwrap(t) {
    ...
    const o = window.getSelection(), e = o.getRangeAt(0), s = e.extractContents();
    this.clickedOnLeft ? this.removeWrapper(t) : this.updateWrapper(t), e.insertNode(s), o.removeAllRanges(), o.addRange(e);
  }
  /**
   ...
   */
  checkState() {
    const t = this.api.selection.findParentTag("SPAN"), o = this.api.selection.findParentTag(this.parentTag), e = t ? this.handleLegacyWrapper(t, o) : o;
    return this.button.classList.toggle(this.iconClasses.active, !!e), !!e;
  }
  /**
   * handle icon active state for legacy wrappers
   */
  handleLegacyWrapper(t, o) {
    return this.pluginType === "marker" ? t : o & t;
  }
  /**
   * Sanitizer rule
